...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        cnt = 0
        while it != beehive:
            if it.bees != [] and cnt >= self.min_range and cnt <= self.max_range:
                return random_or_none(it.bees)
            else:
                it = it.entrance
                cnt += 1
        return None
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        # BEGIN Problem 10
        # Cause'of reriding this, the original func should repeat
        if self.contained_ant:
            self.contained_ant.action(gamestate)
        for bee in self.place.bees[:]:
            bee.reduce_armor(self.damage)
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        while it is not None:
            if it.ant is not None:
                if isinstance(it.ant, ContainerAnt):
                    if it.ant.enhanced is False:
                        it.ant.enhanced = True
                        it.ant.damage *= 2
                    if it.ant.contained_ant is not None:
                        if it.ant.contained_ant.enhanced is False:
                            it.ant.contained_ant.enhanced = True
                            it.ant.contained_ant.damage *= 2
                else:
                    if it.ant.enhanced is False:
                        it.ant.enhanced = True
                        it.ant.damage *= 2
