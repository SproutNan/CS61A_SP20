...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def about_what(element):
        for x in topic:
            if x in lower(remove_punctuation(element)).split():
                return True
        return False
    return about_what
    # END PROBLEM 2
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9
    times = []
    for ele in times_per_player:
        times.append([ele[i+1] - ele[i] for i in range(len(ele)-1)])
